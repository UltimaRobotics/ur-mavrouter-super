...
struct LogOptions {
    ...
    unsigned long min_free_space{100};            // conf "MinFreeSpace"
    unsigned long max_log_files{10};              // conf "MaxLogFiles"
    ...
    // Copy constructor to ensure safe copying
    LogOptions(const LogOptions& other) 
        : logs_dir(other.logs_dir)
        , log_mode(other.log_mode)
        , mavlink_dialect(other.mavlink_dialect)
        , min_free_space(other.min_free_space)
        , max_log_files(other.max_log_files)
        , fcu_id(other.fcu_id)
        , log_telemetry(other.log_telemetry) {}
    
    // Assignment operator to ensure safe assignment
