...
public:
    ...
    // Statistics methods
    EndpointStats::EndpointStatistics* get_statistics() const { return _statistics.get(); }
    void update_statistics_periodic();
...
protected:
    ...
    // Enhanced statistics
    std::unique_ptr<EndpointStats::EndpointStatistics> _statistics;
...
public:
     ...
     * router exit. Default to false for graceful handling.
     */
...
protected:
    ...
    bool _reconnect_cb(void *data);
    std::string _find_alternative_device(const std::string &original_device);
    std::vector<std::string> _scan_available_devices();

public:
    void _handle_epoll_error();

private:
    size_t _current_baud_idx = 0;
    Timeout *_change_baud_timeout = nullptr;
    Timeout *_reconnect_timeout = nullptr;
    std::vector<uint32_t> _baudrates;
    UartEndpointConfig _config;
...
public:
    bool should_retry_connection() const { return _retry_timeout > 0; }
