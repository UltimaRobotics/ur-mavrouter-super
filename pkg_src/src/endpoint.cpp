    ...
    , _name{std::move(name)}
{
    ...
    _statistics = std::make_unique<EndpointStats::EndpointStatistics>(_name);
}
...
int Endpoint::read_msg(struct buffer *pbuf)
{
    ...
    if (msg_entry) {
        ...
        if (!_check_crc(msg_entry)) {
            ...
            if (_statistics) {
                _statistics->message_stats.on_malformed_packet();
                _statistics->log_endpoint_error(EndpointStats::ErrorCategory::PROTOCOL, 
                                     "CRC check failed for message ID " + std::to_string(msg_id));
            }
            return 0;
        }
    }
    ...
    if (_statistics) {
        _statistics->message_stats.on_message_received(expected_size, mavlink2);
        
        // Update buffer utilization
        _statistics->performance.update_buffer_utilization(
            rx_buf.len, RX_BUF_MAX_SIZE,
            tx_buf.len, TX_BUF_MAX_SIZE
        );
    }
...
{
    // CRITICAL FIX: Use the instance() method which returns the correct mainloop
    // For extension threads, this will return their own mainloop instance
    // For main thread, this returns the singleton
    // This prevents the data race that was causing the segfault
    return Mainloop::instance().dedup_check_msg(buf);
}
...
void Endpoint::update_statistics_periodic()
{
    if (_statistics) {
        _statistics->update_periodic_stats();
    }
}

std::string Endpoint::get_statistics_json() const
{
    if (_statistics) {
        return _statistics->to_json();
    }
    return "{}";
}
...
bool UartEndpoint::setup(UartEndpointConfig conf)
{
    ...
    // Initialize UART-specific statistics
    if (_statistics) {
        _statistics->initialize_uart_stats();
        _statistics->connection_health.set_state(EndpointStats::ConnectionState::CONNECTING);
    }
...
int UartEndpoint::set_speed(speed_t baudrate)
{
    ...
    // Update statistics
    if (_statistics && _statistics->uart_stats) {
        _statistics->uart_stats->on_baudrate_change(baudrate);
    }
...
set_latency_failed:
    ...
    if (_statistics) {
        _statistics->connection_health.on_connection_established();
        if (_statistics->uart_stats) {
            _statistics->uart_stats->add_device_path(path);
        }
    }
...
void UartEndpoint::handle_connection_lost()
{
    log_warning("UART %s: Connection lost, scheduling reconnection", _name.c_str());
    
    // Update statistics
    if (_statistics) {
        _statistics->connection_health.on_connection_lost();
        _statistics->log_endpoint_error(EndpointStats::ErrorCategory::HARDWARE, 
                              "UART connection lost");
    }
    
    if (fd >= 0) {
        Mainloop::get_instance().remove_fd(fd);
        Mainloop::get_instance().untrack_fd(fd);
        ::close(fd);
        fd = -1;
    }
    _valid = false;
...
void UartEndpoint::schedule_reconnect()
{
    if (_reconnect_timeout == nullptr) {
        _reconnect_timeout = Mainloop::get_instance().add_timeout(
            5000, // 5 second retry interval
            std::bind(&UartEndpoint::_reconnect_cb, this, std::placeholders::_1),
            this);
    }
}

bool UartEndpoint::_reconnect_cb(void *data)
{
    static int retry_count = 0;
    const int max_retries = 10;
    
    log_info("UART %s: Attempting to reconnect (attempt %d/%d)", 
             _name.c_str(), retry_count + 1, max_retries);
    
    // First try the original device
    if (open(_config.device.c_str())) {
        // Reapply configuration
        if (_config.baudrates.size() == 1) {
            set_speed(_config.baudrates[0]);
        } else {
            add_speeds(_config.baudrates);
        }
        ...
        log_info("UART %s: Successfully reconnected to original device", _name.c_str());
        _reconnect_timeout = nullptr;
        retry_count = 0; // Reset retry count on success
        return false; // Stop retrying
    }
    
    // Scan for all available devices (fresh scan each time)
    std::vector<std::string> available_devices = _scan_available_devices();
    
    // Try each available device that's not the original
    for (const auto &device : available_devices) {
        if (device == _config.device) {
            continue; // Skip original device as we already tried it
        }
        
        log_info("UART %s: Trying alternative device %s", _name.c_str(), device.c_str());
        
        if (open(device.c_str())) {
            // Update config with new device path
            std::string old_device = _config.device;
            ...
            _valid = true;
            Mainloop::get_instance().add_fd(fd, this, EPOLLIN);
            log_info("UART %s: Successfully connected to alternative device %s (was %s)", 
                     _name.c_str(), device.c_str(), old_device.c_str());
            _reconnect_timeout = nullptr;
            retry_count = 0; // Reset retry count on success
            return false; // Stop retrying
        }
    ...
    if (retry_count >= max_retries) {
        log_error("UART %s: Maximum reconnection attempts (%d) reached, giving up", 
                  _name.c_str(), max_retries);
        _valid = false;
        _reconnect_timeout = nullptr;
        retry_count = 0;
        return false; // Stop retrying
    ...
    log_warning("UART %s: Reconnection attempt %d/%d failed, will retry (scanned %zu devices)", 
                _name.c_str(), retry_count, max_retries, available_devices.size());
...
ssize_t UartEndpoint::_read_msg(uint8_t *buf, size_t len)
{
    ...
    if (r == -1) {
        if (errno == EBADF || errno == EIO || errno == ENXIO) {
            log_warning("UART %s: Device disconnected (errno=%d)", _name.c_str(), errno);
...
{
    if (fd < 0) {
        log_debug("UART %s: Trying to write to invalid fd, skipping", _name.c_str());
        return 0; // Don't treat as error, just skip
    }
    ...
    if (r == -1) {
        if (errno == EBADF || errno == EIO || errno == ENXIO) {
            log_warning("UART %s: Device disconnected during write (errno=%d)", _name.c_str(), errno);
            handle_connection_lost();
        }
        return -errno;
    }
...
std::vector<std::string> UartEndpoint::_scan_available_devices()
{
    std::vector<std::string> devices;
    
    // Scan /dev directory for ttyACM* and ttyUSB* devices
    DIR *dir = opendir("/dev");
    if (!dir) {
        log_error("UART %s: Failed to open /dev directory", _name.c_str());
        return devices;
    }
    ...
    while ((entry = readdir(dir)) != nullptr) {
        if (strncmp(entry->d_name, "ttyACM", 6) == 0 || 
            ...
            if (stat(device_path.c_str(), &st) == 0 && S_ISCHR(st.st_mode)) {
                // Try to open briefly to check if it's accessible
                int test_fd = ::open(device_path.c_str(), O_RDWR | O_NONBLOCK | O_CLOEXEC | O_NOCTTY);
                if (test_fd >= 0) {
                    ::close(test_fd);
                    devices.push_back(device_path);
                    log_debug("UART %s: Found available device %s", _name.c_str(), device_path.c_str());
                }
            }
        }
    }
...
std::string UartEndpoint::_find_alternative_device(const std::string &original_device)
{
    std::vector<std::string> available_devices = _scan_available_devices();
    
    // Find first device that's not the original
    for (const auto &device : available_devices) {
        if (device != original_device) {
            log_info("UART %s: Found alternative device %s", _name.c_str(), device.c_str());
            return device;
        }
    }
    
    log_debug("UART %s: No alternative devices found (scanned %zu devices)", 
              _name.c_str(), available_devices.size());
    return "";
}

void UartEndpoint::_handle_epoll_error()
{
    log_warning("UART %s: EPOLL error detected, cleaning up fd %d", _name.c_str(), fd);
    
    // Remove from epoll to prevent further error events
    if (fd >= 0) {
        Mainloop::get_instance().remove_fd(fd);
        ::close(fd);
        fd = -1;
...
bool UdpEndpoint::setup(UdpEndpointConfig conf)
{
    ...
    // Initialize UDP-specific statistics
    if (_statistics) {
        _statistics->initialize_udp_stats();
        _statistics->connection_health.set_state(EndpointStats::ConnectionState::CONNECTING);
    }

    if (!this->open(conf.address.c_str(), conf.port, conf.mode)) {
        log_warning("UDP %s: Initial connection failed to %s:%ld, will retry", 
                   _name.c_str(), conf.address.c_str(), conf.port);
        schedule_reconnect();
        return true; // Don't fail setup, let reconnection handle it
    }
...
void UdpEndpoint::handle_connection_lost()
{
    log_warning("UDP %s: Connection lost, scheduling reconnection", _name.c_str());
    if (fd >= 0) {
        Mainloop::instance().untrack_fd(fd);
        ::close(fd);
        fd = -1;
    }
    _valid = false;
...
void UdpEndpoint::schedule_reconnect()
{
    if (_reconnect_timeout == nullptr) {
        _reconnect_timeout = Mainloop::get_instance().add_timeout(
            5000, // 5 second retry interval
            std::bind(&UdpEndpoint::_reconnect_cb, this, std::placeholders::_1),
            this);
    }
}

bool UdpEndpoint::_reconnect_cb(void *data)
{
    log_info("UDP %s: Attempting to reconnect to %s:%ld", 
             _name.c_str(), _config.address.c_str(), _config.port);
    
    if (open(_config.address.c_str(), _config.port, _config.mode)) {
        _valid = true;
        Mainloop::get_instance().add_fd(fd, this, EPOLLIN);
        log_info("UDP %s: Successfully reconnected", _name.c_str());
        _reconnect_timeout = nullptr;
        return false; // Stop retrying
    }
    
    log_warning("UDP %s: Reconnection attempt failed, will retry", _name.c_str());
    return true; // Continue retrying
}
...
ssize_t UdpEndpoint::_read_msg(uint8_t *buf, size_t len)
{
    ...
    if (r == -1) {
        if (errno == EBADF || errno == ECONNRESET || errno == ENETUNREACH) {
            log_warning("UDP %s: Network error (errno=%d), scheduling reconnection", _name.c_str(), errno);
            handle_connection_lost();
        }
        return -errno;
    }
...
int UdpEndpoint::write_msg(const struct buffer *pbuf)
{
    ...
    if (fd < 0) {
        log_debug("UDP %s: Trying to write to invalid fd, skipping", _name.c_str());
        return 0; // Don't treat as error, just skip
    }
    ...
    if (r == -1) {
        if (errno == EBADF || errno == ECONNRESET || errno == ENETUNREACH) {
            log_warning("UDP %s: Network error during write (errno=%d), scheduling reconnection", 
                       _name.c_str(), errno);
            handle_connection_lost();
        } else if (errno != EAGAIN && errno != ECONNREFUSED) {
            log_warning("UDP %s: Error sending udp packet (%m)", _name.c_str());
        }
        return -errno;
    };
...
bool TcpEndpoint::setup(TcpEndpointConfig conf)
{
    ...
    // Initialize TCP-specific statistics
    if (_statistics) {
        _statistics->initialize_tcp_stats();
        _statistics->connection_health.set_state(EndpointStats::ConnectionState::CONNECTING);
    }
...
bool TcpEndpoint::open(const std::string &ip, unsigned long port)
{
    ...
    if (_statistics) {
        _statistics->connection_health.on_connection_established();
        if (_statistics->tcp_stats) {
            _statistics->tcp_stats->on_connection_start();
        }
    }
